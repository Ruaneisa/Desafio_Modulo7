...
function addBook() {
    ...
    const book = servicos.addBook(title, author, year, genre);
    console.log('Livro adicionado:', book);
}

function listBooks() {
    const books = servicos.listBooks();
    if (books.length < 1) {
...
function updateBook() {
    ...
    const book = servicos.updateBook(id, updatedData);
    if (book) {
...
function deleteBook() {
    ...
    const book = servicos.deleteBook(id);
    if (book) {
...
`);
    ...
    switch (searchType) {
        case '1':
            const title = prompt('Título: ');
            console.log('Livros encontrados:', servicos.findBooksByTitle(title));
            break;
        case '2':
            const author = prompt('Autor: ');
            console.log('Livros encontrados:', servicos.findBooksByAuthor(author));
            break;
        case '3':
            const year = parseInt(prompt('Ano: '), 10);
            console.log('Livros encontrados:', servicos.findBooksByYear(year));
            break;
        case '4':
            const genre = prompt('Gênero: ');
            console.log('Livros encontrados:', servicos.findBooksByGenre(genre));
            break;
        default:
