...
const { createStudent } = require('./perfil');
...
function updateStudent(rl, callback) {
  rl.question('Acesse com o ID do Estudante para Atualizar: ', id => {
    const student = estudante.find(students => students.id === id);
    if (!student) {
...
function deleteStudent(rl, callback) {
  rl.question('Informe o ID do Estudante para ser deletado: ', id => {
    const index = estudante.findIndex(students => students.id === id);
    if (index === -1) {
      console.log('Estudante não encontrado');
      callback();
      return;
    ...
    switch (option) {
      case '1':
        rl.question('Entre com o nome que procura:  ', name => {
          const result = estudante.filter(student => student.name.toLowerCase().includes(name.toLowerCase()));
          if (result.length > 0) {
      ...
      case '2':
        rl.question('Insira a Matricula: ', matricula => {
          const result = estudante.filter(student => student.matricula.includes(matricula));
          if (result.length > 0) {
            console.log('Estudantes encontrados:');
            console.log(result);
          } else {
      ...
      case '3':
        rl.question('Insira o curso: ', curso => {
          const result = estudante.filter(student => student.curso.toLowerCase().includes(curso.toLowerCase()));
          if (result.length > 0) {
            console.log('Estudantes encontrados:');
            console.log(result);
          } else {
