const { createProduct }  = require('./produto');
...
function updateProduct(rl, callback) {
  rl.question('Acesse com o ID do Produto para Atualizar: ', id => {
    const product = produto.find(products => products.id === id);
    if (!product) {
...
function deleteProduct(rl, callback) {
  rl.question('Informe o ID do Produto para ser deletado: ', id => {
    const index = produto.findIndex(products => products.id === id);
    if (index === -1) {
      console.log('Produto não encontrado');
      callback();
      return;
    ...
    switch (option) {
      case '1':
        rl.question('Entre com o Nome que procura:  ', name => {
          const result = produto.filter(product => product.name.toLowerCase().includes(name.toLowerCase()));
          if (result.length > 0) {
      ...
      case '2':
        rl.question('Qual Categoria procura : ', categoria => {
          const result = produto.filter(product => product.categoria.includes(categoria));
          if (result.length > 0) {
      ...
      case '3':
        rl.question('Insira o Preço: ', preco => {
          const result = produto.filter(product => product.preco === preco);
          if (result.length > 0) {
            console.log('Produtos encontrados:');
            console.log(result);
          } else {
      ...
      case '4':
        rl.question('Insira a Quantidade em Estoque: ', estoque => {
          const result = produto.filter(product => product.estoque);
          if (result.length >= 0) {
            console.log('Produto encontrados:');
            console.log(result);
          } else {
